...
//       updateMapMarkers();
//     }
//   }, [filteredData, mapReady, showHeatmap, radiusFilter]);
...
//           break;
//         case 'markerClick':
//           const { markerId, markerType } = message.data;
//           const marker = markerType === 'dog' 
//             ? dogs.find(d => d.id === markerId)
//             : emergencies.find(e => e.id === markerId);
//           if (marker) {
//             onMarkerPress(marker, markerType);
//           }
//           break;
//         case 'clusterClick':
...
//   const getMapHTML = () => {
//     const getTileLayer = (style: MapStyle) => {
//       switch (style) {
//         case 'satellite':
...
//                 font-weight: bold;
//             }
//             .dog-marker { background-color: #3b82f6; }
//             .stray-marker { background-color: #f97316; }
//             .emergency-critical { background-color: #ef4444; }
//             .emergency-high { background-color: #f97316; }
//             .emergency-medium { background-color: #eab308; }
//             .emergency-low { background-color: #84cc16; }
//             .cluster-marker {
...
//                 currentTileLayer = L.tileLayer('${getTileLayer(mapStyle)}', {
...
//                 if (type === 'dog') {
//                     iconClass = item.dog_type === 'stray' ? 'stray-marker' : 'dog-marker';
//                     iconText = '🐕';
//                 } else {
//                     iconClass = 'emergency-' + item.severity;
//                     iconText = '⚠️';
//                 }
                
//                 const icon = L.divIcon({
//                     html: '<div class="custom-marker ' + iconClass + '">' + iconText + '</div>',
...
//                     window.ReactNativeWebView.postMessage(JSON.stringify({
//                         type: 'markerClick',
//                         data: { markerId: item.id, markerType: type }
//                     }));
//                 });
...
//                         icon: L.divIcon({
//                             html: '<div style="width: 20px; height: 20px; border-radius: 10px; background-color: #10b981; border: 3px solid white; box-shadow: 0 2px 8px rgba(0,0,0,0.3);"></div>',
...
//                     // Add radius circle
//                     radiusCircle = L.circle([data.userLocation.latitude, data.userLocation.longitude], {
//                         radius: data.radiusFilter * 1000,
//                         color: '#10b981',
//                         fillColor: '#10b981',
//                         fillOpacity: 0.1,
//                         weight: 2,
...
//                     const marker = createMarker(dog, 'dog');
//                     markersLayer.addLayer(marker);
//                 });
                
//                 // Add emergency markers
//                 data.emergencies.forEach(emergency => {
//                     const marker = createMarker(emergency, 'emergency');
//                     markersLayer.addLayer(marker);
//                 });
                
//                 // Add heatmap if enabled
//                 if (data.showHeatmap) {
//                     const heatmapData = [...data.dogs, ...data.emergencies].map(item => [
...
//                 // Change tile layer if needed
//                 const newTileUrl = getTileUrl(data.mapStyle);
//                 if (currentTileLayer.getTileUrl(null) !== newTileUrl) {
...
//                 <Pressable
//                   onPress={() => setRadiusFilter(Math.max(5, radiusFilter - 10))}
//                   className="bg-gray-300 px-3 py-1 rounded-full"
//                 >
//                   <Text className="text-xs text-gray-600">-</Text>
//                 </Pressable>
//                 <Pressable
//                   onPress={() => setRadiusFilter(Math.min(100, radiusFilter + 10))}
//                   className="bg-gray-300 px-3 py-1 rounded-full"
//                 >
//                   <Text className="text-xs text-gray-600">+</Text>
//                 </Pressable>
